...
const chartDetails = async (req, res) => {
  try {
    ...
    const appoPending = await AppointmentModel.countDocuments({
      doctor: drId,
      status: "Pending",
    });
    const appoDone = await AppointmentModel.countDocuments({
      doctor: drId,
      status: "Done",
    });
    const appoCancelled = await AppointmentModel.countDocuments({
      doctor: drId,
      status: "Cancelled",
    });
